...
- 从常见小说、漫画网站导入作品
- 禁漫可直接输入纯数字本子号：电脑端在“添加书籍”的地址栏输入；手机端选择“导入作品 → 禁漫本子号”。自动识别为漫画，获取章节、下载并还原图片后加入书架；任务可收起并继续查看进度。
...
18Comic、Bika、E-Hentai 等站点，也支持导入 Legado / 阅读 App JSON 书源。搜索页可选择“书源”“夸克”“番茄”“起点”或“笔趣阁”；内置解析器会保留上游可发现的章节，并在上游返回可解析内容时默认保存，不因 VIP、订阅或付费标记提前跳过。笔趣阁聚合会自动发现八零小说网当前搜索入口；b520 与笔趣看的站内搜索已下线，改用公开目录索引，因此结果和目录可能不完整，笔趣看还可能受部署区域限制。未实现章节解析或上游未返回可用内容时会明确提示。Windows 本机模式可在客户端“插件配置”中导入、更新和卸载独立插件包，并启停内置解析器；
Linux 远程模式在服务器管理界面的“插件管理”中统一维护，客户端会自动隐藏该入口。Legado 规则仍在“书源管理”中管理。导入插件可直接用于链接解析、下载和搜索页“导入插件”搜索。开发、打包与运行规则见[站点插件规范 v1](./docs/development/07-site-plugin-spec.md)，附带可直接运行的离线示例。
...
“服务概览”还提供开启、关闭和重启业务服务。关闭后客户端业务请求与尚未开始的队列任务会暂停，当前正在执行的任务会安全结束；独立的管理通道保持在线，因此管理员仍可查看状态并重新开启服务。
