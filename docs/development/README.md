...
- **FastAPI 后端**：有两种运行形态——
  ...
  - *Linux 远程服务*：systemd 单进程多用户服务，供 Android 和 Windows 远程连接；Android 也可连接 PC 局域网共享入口。
- **管理界面**：随 Linux 后端一起提供的 React 管理页（`/admin/`），用于配置服务器、维护用户、查看设备 / 诊断 / 日志等运维操作。它不是阅读客户端。

> 一句话总结：**“当前选中的后端”是书籍、任务、阅读进度、模型设置的唯一权威来源**；客户端展示与操作，并可保存按服务实例和账号隔离的离线副本、待提交进度与听书位置。
...
| [书库元数据](./09-library-metadata.md) | 分组、标签、置顶、覆盖信息与冲突保护 | 书库与详情 |
| [连载追更](./10-serial-updates.md) | 目录增量、定时检查、自动下载与未读状态 | 后端任务、书库与详情 |
| [书签、笔记与搜索](./11-reading-annotations-search.md) | 账号隔离、版本保护、已缓存正文搜索 | 阅读器与后端 |
| [空间与资源配额](./12-storage-management.md) | 文件占用、清理确认、用户额度和任务调度 | 详情、管理界面与后端 |
| [翻译质量](./13-translation-quality.md) | 术语、校对、版本历史、选段重译和用量 | 小说详情与后端 |
...
- **数据由后端管**：书库、任务、阅读进度、插件开关都在后端，客户端可保存界面偏好、安全连接信息、隔离的设备离线副本和位置记录，不能据此覆盖服务端权威状态。

## 4. 平台边界一句话

| | Windows 客户端 | Android 客户端 | Linux 后端 |
| --- | --- | --- | --- |
| 连接方式 | 本机后端(127.0.0.1) **或** 远程 Linux | Linux 服务或 PC 局域网共享 | 本地多用户服务 |
...
> 手机端仍不需要 Python 环境——手机只连接 Linux 服务或 PC 共享入口，不运行本机 Python。

---

## 6. 当前发布基线（读代码前先看这个）

- 已发布版本：**v2.2.1（build 43）**；仓库当前发布候选：**`2.3.0+44`**（书库管理、连载追更、阅读标注与离线缓存、备份恢复、存储配额、插件维护和翻译质量管理）。
...
1. **组件体系隔离**：Windows 使用 `fluent_ui`；Android 使用统一移动主题下的 Material、`flutter_miuix` 和经适配的兼容组件；管理界面使用 Ant Design。共享业务逻辑，按平台构建界面，具体约束见[UI 与可访问性](./02-ui-and-accessibility.md)。
