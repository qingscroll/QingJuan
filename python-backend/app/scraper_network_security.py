...
async def resolve_public_url(
    ...
    try:
        addresses = await (resolver or resolve_model_endpoint_addresses)(host, port)
...
class PublicHTTPTransport(ValidatedModelHTTPTransport):
    def __init__(self, *, resolver: AddressResolver | None = None) -> None:
        ...
        super().__init__(allowlist=frozenset(), resolver=resolver)
    ...
    def __init__(
        ...
        *,
        ...
        self._backend = ValidatedModelNetworkBackend(
            allowlist=frozenset(), resolver=resolver, delegate=delegate
        )
